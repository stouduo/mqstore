...
public class Node<T> {
    ...
    public void insertOrUpdate(Comparable key, T obj, BplusTree tree) {
        //如果是叶子节点
        if (isLeaf) {
            //不需要分裂，直接插入或更新
            if (contains(key) || entries.size() < tree.getOrder()) {
                ...
                if (parent != null) {
                    ...
                    isRoot = false;
                    Node<T> parent = new Node(false, true);
                    tree.setRoot(parent);
                    left.setParent(parent);
                    right.setParent(parent);
    ...
    protected void updateInsert(BplusTree tree) {
        ...
        if (children.size() > tree.getOrder()) {
            //分裂成左右两个节点
            Node<T> left = new Node(false);
            Node<T> right = new Node(false);
            //左右两个节点关键字长度
            int leftSize = (tree.getOrder() + 1) / 2 + (tree.getOrder() + 1) % 2;
            int rightSize = (tree.getOrder() + 1) / 2;
     ...
     * 调整节点关键字
     */
    protected static <T> void validate(Node<T> node, BplusTree tree) {
     ...
     * 插入到当前节点的关键字中
     */
    protected void insertOrUpdate(Comparable key, T obj) {
